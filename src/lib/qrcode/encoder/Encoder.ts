...
export default class Encoder {
  ...
  public static encode(content: string,
    ...
    if (hints !== null && hints !== undefined && undefined !== hints.get(EncodeHintType.QR_VERSION)) {
      ...
      if (!this.willFit(bitsNeeded, version, ecLevel)) {
        throw new Exception("WriterException", "Data too big for requested version")
      }
    } else {
  ...
  public static terminateBits(numDataBytes: number/*int*/, bits: BitArray): void /*throws WriterException*/ {
    ...
    if (bits.getSize() > capacity) {
      throw new Exception("WriterException", "data bits cannot fit in the QR Code" + bits.getSize() + " > " +
          capacity)
    }
  ...
  public static appendNumericBytes(content: string, bits: BitArray): void {
    ...
    let i = 0
    while (i < length) {
      const num1 = StringEncoding.getDigit(content.charAt(i))
      if (i + 2 < length) {
        // Encode three numeric letters in ten bits.
        const num2 = StringEncoding.getDigit(content.charAt(i + 1))
        const num3 = StringEncoding.getDigit(content.charAt(i + 2))
  ...
  public static append8BitBytes(content: string, bits: BitArray, encoding: string): void
    ...
    try {
      ...
      throw new Exception("WriterException", uee)
    }
