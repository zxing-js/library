...
export default /*final*/ class State {
  ...
  static /*final*/ INITIAL_STATE: State = new State(Token.EMPTY, HighLevelEncoder.MODE_UPPER, 0, 0);
  ...
  latchAndAppend(mode: int, value: int): State {
    ...
    if (mode !== this.mode) {
      let latch: int = HighLevelEncoder.LATCH_TABLE[this.mode][mode];
      token = token.add(latch & 0xFFFF, latch >> 16);
      bitCount += latch >> 16;
    }
    let latchModeBitCount: int = mode === HighLevelEncoder.MODE_DIGIT ? 4 : 5;
  ...
  shiftAndAppend(mode: int, value: int): State {
    ...
    let token: Token = this.token;
    let thisModeBitCount: int = this.mode === HighLevelEncoder.MODE_DIGIT ? 4 : 5;
  ...
  addBinaryShiftChar(index: int): State {
    ...
    if (this.mode === HighLevelEncoder.MODE_PUNCT || this.mode === HighLevelEncoder.MODE_DIGIT) {
      // assert binaryShiftByteCount === 0;
      let latch: int = HighLevelEncoder.LATCH_TABLE[mode][HighLevelEncoder.MODE_UPPER];
      token = token.add(latch & 0xFFFF, latch >> 16);
      bitCount += latch >> 16;
      mode = HighLevelEncoder.MODE_UPPER;
    }
    let deltaBitCount: int =
      (this.binaryShiftByteCount === 0 || this.binaryShiftByteCount === 31) ? 18 :
        (this.binaryShiftByteCount === 62) ? 9 : 8;
  ...
  isBetterThanOrEqualTo(other: State): boolean {
    let newModeBitCount: int = this.bitCount + (HighLevelEncoder.LATCH_TABLE[this.mode][other.mode] >> 16);
  ...
  public toString(): String {
    return StringUtils.format('%s bits=%d bytes=%d', HighLevelEncoder.MODE_NAMES[this.mode], this.bitCount, this.binaryShiftByteCount);
  }
