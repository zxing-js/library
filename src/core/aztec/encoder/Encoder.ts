...
export default /*public final*/ class Encoder {
    ...
    private static /*final*/ WORD_SIZE: Int32Array = Int32Array.from([4, 6, 6, 8, 8, 8, 8, 8, 8, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12]);
    ...
    public static encode(data: Uint8Array, minECCPercent: int, userSpecifiedLayers: int): AztecCode {
        ...
        let eccBits: int = Math.trunc((bits.getSize() * minECCPercent) / 100) + 11;
        ...
        if (compact) {
            ...
            let origCenter: int = Math.trunc(baseMatrixSize / 2);
            let center: int = Math.trunc(matrixSize / 2);
            for (let i /*int*/ = 0; i < origCenter; i++) {
                let newOffset: int = i + Math.trunc(i / 15);
        ...
        if (compact) {
            Encoder.drawBullsEye(matrix, Math.trunc(matrixSize / 2), 5);
        } else {
            Encoder.drawBullsEye(matrix, Math.trunc(matrixSize / 2), 7);
            for (let i /*int*/ = 0, j = 0; i < Math.trunc(baseMatrixSize / 2) - 1; i += 15, j += 16) {
                for (let k /*int*/ = Math.trunc(matrixSize / 2) & 1; k < matrixSize; k += 2) {
                    matrix.set(Math.trunc(matrixSize / 2) - j, k);
                    matrix.set(Math.trunc(matrixSize / 2) + j, k);
                    matrix.set(k, Math.trunc(matrixSize / 2) - j);
                    matrix.set(k, Math.trunc(matrixSize / 2) + j);
                }
            }
        }
    ...
    private static drawModeMessage(matrix: BitMatrix, compact: boolean, matrixSize: int, modeMessage: BitArray): void {
        ...
        if (compact) {
            ...
            for (let i /*int*/ = 0; i < 10; i++) {
                let offset: int = center - 5 + i + Math.trunc(i / 5);
                if (modeMessage.get(i)) {
    ...
    private static generateCheckWords(bitArray: BitArray, totalBits: int, wordSize: int): BitArray {
        ...
        let totalWords: int = Math.trunc(totalBits / wordSize);
