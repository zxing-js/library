...
export /*public final*/ class Encoder {
  ...
  public static encode(data: Uint8Array, minECCPercent: int, userSpecifiedLayers: int): AztecCode {
    ...
    let eccBits: int = bits.getSize() * minECCPercent / 100 + 11;
    ...
    if (compact) {
      ...
      let origCenter: int = baseMatrixSize / 2;
      let center: int = matrixSize / 2;
      for (let i /*int*/ = 0; i < origCenter; i++) {
        let newOffset: int = i + i / 15;
  ...
  private static generateCheckWords(bitArray: BitArray, totalBits: int, wordSize: int): BitArray {
    ...
    let totalWords: int = totalBits / wordSize;
