...
export default class FinderPatternFinder {
  ...
  private static /* final*/  CENTER_QUORUM: int = 2;
  private static /* final*/  moduleComparator: EstimatedModuleComparator = new EstimatedModuleComparator();
  protected static /* final*/  MIN_SKIP: int = 3; // 1 pixel/module times 3 modules/center
  protected static /* final*/  MAX_MODULES: int = 97; // support up to version 20 for mobile clients

  private /* final*/  image: BitMatrix;
  private /* final*/  possibleCenters: List<FinderPattern>;
  private hasSkipped: boolean;
  private /* final*/  crossCheckStateCount: Int32Array;
   ...
   */
  private constructorOverload1(image: BitMatrix) {
   ...
   */
  private constructorOverload2(image: BitMatrix, resultPointCallback: ResultPointCallback) {
    this.image = image;
  ...
  protected /* final */  getImage(): BitMatrix {
    return this.image;
  }

  protected /* final */  getPossibleCenters(): List<FinderPattern> {
   ...
   */
  /* final */ find(hints: Map<DecodeHintType, any>): FinderPatternInfo {
    const tryHarder: boolean = hints != null && hints.has(DecodeHintType.TRY_HARDER);
    const maxI: int = this.image.getHeight();
    const maxJ: int = this.image.getWidth();
    ...
    let iSkip: int = Math.trunc((3 * maxI) / (4 * FinderPatternFinder.MAX_MODULES));
    ...
    for (let i: int = iSkip - 1; i < maxI && !done; i += iSkip) {
          ...
          if ((currentState & 1) === 0) { // Counting black pixels
            if (currentState === 4) { // A winner?
              if (FinderPatternFinder.foundPatternCross(stateCount)) { // Yes
                let confirmed: boolean = this.handlePossibleCenter(stateCount, i, j);
                if (confirmed) {
      ...
      if (FinderPatternFinder.foundPatternCross(stateCount)) {
        let confirmed: boolean = this.handlePossibleCenter(stateCount, i, maxJ);
        if (confirmed) {
          iSkip = stateCount[0];
          if (this.hasSkipped) {
            // Found a third one
   ...
   */
  private static centerFromEnd(stateCount: Int32Array, end: int): float {
  ...
  protected static foundPatternCross(stateCount: Int32Array): boolean {
    let totalModuleSize: int = 0;
    for (let i: int = 0; i < 5; i++) {
      let count: int = stateCount[i];
      if (count === 0) {
        return false;
      }
    ...
    const moduleSize: float = totalModuleSize / 7.0; // TYPESCRIPTPORT: check if a precision reduction is needed
    const maxVariance: float = moduleSize / 2.0; // TYPESCRIPTPORT: check if a precision reduction is needed
  ...
  protected static foundPatternDiagonal(stateCount: Int32Array): boolean {
    ...
    const moduleSize: float = totalModuleSize / 7.0;
    const maxVariance: float = moduleSize / 1.333;
  ...
  protected /* final */  clearCounts(counts: Int32Array): void {
    Arrays.fill(counts, 0);
  }

  protected /* final */  shiftCounts2(stateCount: Int32Array): void {
   ...
   */
  private crossCheckDiagonal(centerI: int, centerJ: int): boolean {
   ...
   */
  private crossCheckVertical(startI: int, centerJ: int, maxCount: int,
    originalStateCountTotal: int): float {
    const image: BitMatrix = this.image;

    const maxI: int = image.getHeight();
    let stateCount: Int32Array = this.getCrossCheckStateCount();
    ...
    const stateCountTotal: int = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] +
      stateCount[4];
   ...
   */
  private crossCheckHorizontal(startJ: int, centerI: int, maxCount: int,
    originalStateCountTotal: int): float {
    const image: BitMatrix = this.image;
   ...
   * @Deprecated
   */
  protected /* final */  handlePossibleCenterX(stateCount: Int32Array, i: int, j: int, pureBarcode: boolean): boolean {
   ...
   * @return true if a finder pattern candidate was found this time
   */
  protected /* final */ handlePossibleCenter(stateCount: Int32Array, i: int, j: int): boolean {
    const stateCountTotal: int = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] +
      stateCount[4];
    let centerJ: float = FinderPatternFinder.centerFromEnd(stateCount, j);
    const centerI: float = this.crossCheckVertical(i, <int>centerJ, stateCount[2], stateCountTotal);
    if (!Float.isNaN(centerI)) {
      // Re-cross check
      centerJ = this.crossCheckHorizontal(<int>Math.trunc(centerJ), <int>Math.trunc(centerI), stateCount[2], stateCountTotal);
      if (!Float.isNaN(centerJ) && this.crossCheckDiagonal(<int>Math.trunc(centerI), <int>Math.trunc(centerJ))) {
        const estimatedModuleSize: float = stateCountTotal / 7.0;
        let found: boolean = false;
        for (let index: int = 0; index < this.possibleCenters.length; index++) {
          const center: FinderPattern = this.possibleCenters[index];
  ...
  private findRowSkip(): int {
    const max: int = this.possibleCenters.length;
    if (max <= 1) {
      return 0;
    }
    ...
    for (const center/*: FinderPattern*/ of this.possibleCenters) {
      if (center.getCount() >= FinderPatternFinder.CENTER_QUORUM) {
        if (firstConfirmedCenter == null) {
          ...
          return <int> /* TYPESCRIPTPORT: Math.trunc here to emulate Java's `int` cast see CONTRIBUTING */ Math.trunc((Math.abs(firstConfirmedCenter.getX() - center.getX()) -
  ...
  private haveMultiplyConfirmedCenters(): boolean {
    ...
    const average: float = totalModuleSize / max;
    let totalDeviation: float = 0.0;
    for (const pattern/*: FinderPattern*/ of this.possibleCenters) {
   ...
   */
  private static squaredDistance(a: FinderPattern, b: FinderPattern): double {
    const x: double = a.getX() - b.getX();
    const y: double = a.getY() - b.getY();
    return x * x + y * y;
  }
  ...
  private selectBestPatterns(): FinderPattern[] {
    ...
    const startSize: int = this.possibleCenters.length;
    if (startSize < 3) {
    ...
    for (let i /*int*/ = 0; i < this.possibleCenters.length - 2; i++) {
      ...
      const minModuleSize: float = fpi.getEstimatedModuleSize();

      for (let j /*int*/ = i + 1; j < this.possibleCenters.length - 1; j++) {
        const fpj: FinderPattern = this.possibleCenters[j];
        const squares0: double = FinderPatternFinder.squaredDistance(fpi, fpj);

        for (let k /*int*/ = j + 1; k < this.possibleCenters.length; k++) {
          const fpk: FinderPattern = this.possibleCenters[k];
          const maxModuleSize: float = fpk.getEstimatedModuleSize();
          ...
          const d: double = Math.abs(squares[2] - 2 * squares[1]) + Math.abs(squares[2] - 2 * squares[0]);
          if (d < distortion) {
            distortion = d;
            bestPatterns[0] = fpi;
            bestPatterns[1] = fpj;
